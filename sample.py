...
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")# データの理解と前処理、データの可視化、データの分析の処理を行うための関数を定義
...
def prepare_dataset_beta(num_partitions: int,
    ...
    for i, folder in enumerate(folders, 1):
        folder_path='data_of_client_folders/'+folder
        client_file = 'data_of_client/'+folder+'.csv'
        merge(folder_path,client_file)

        #標準化処理をどうするか問題！！！！！！！！！！！！！！！、いったんしない方向で考える
        X, Y, df = base(client_file,csv_data = 'csv_data/'+folder+'.csv')
        print("---------------------------------")
